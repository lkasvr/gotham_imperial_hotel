...
var openDatabase = function () {
  return new Promise(function (resolve, reject) {
    ...
    request.onupgradeneeded = function (event) {
      ...
      if (!db.objectSoreNames.contains("reservations")) {
    ...
    openDatabase().then(function (db) {
      openObjectStore(db, storeName, "readwrite")
        .openCursor().onsuccess = function (event) {
          ...
          if (!cursor) reject("Reservation not found in object store");
...
var getReservations = function () {
  return new Promise(function (resolve, reject) {
    openDatabase().then(function (db) {
      var objecjStore = openObjectStore(db, "reservations");
      var reservations = [];
      objecjStore.openCursor().onsuccess = function (event) {
        var cursor = event.target.result;
        if (cursor) {
